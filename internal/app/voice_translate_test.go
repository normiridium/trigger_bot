...
func TestBuildVOTCLITranslateArgsRejectsInvalidLivelyConfiguration(t *testing.T) {
	...
	t.Setenv("VOT_LIVELY_API_TOKEN", "test-oauth-token")
	if _, err := buildVOTCLITranslateArgs("source", "/tmp/out", "translated", "nl", "kk", votProviderYandexLively); err == nil || !strings.Contains(err.Error(), "to ru or en") {
...
func TestVoiceTranslateLivelyKeyboardOffersSupportedSources(t *testing.T) {
	...
	if strings.Contains(joined, "|auto") || !strings.Contains(joined, "vtr|lang|mix|token|ru") || !strings.Contains(joined, "vtr|lang|mix|token|ja") {
