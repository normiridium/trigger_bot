...
func buildVOTCLITranslateArgs(sourcePath, outputDir, outputFile, srcLang, resLang string, provider votProvider) ([]string, error) {
	...
	if provider == votProviderYandexLively {
		if to != "ru" && to != "en" {
			return nil, fmt.Errorf("lively voice supports only translations to ru or en")
		}
...
func handleVoiceTranslateOptionCallback(bot *tgbotapi.BotAPI, cb *tgbotapi.CallbackQuery, q *voiceTranslateQueue) bool {
	...
	case voiceTranslateActionLang:
		...
		if provider == votProviderYandexLively {
			if resLang != "ru" && resLang != "en" {
				_, _ = bot.Request(tgbotapi.NewCallbackWithAlert(cb.ID, "Живые голоса поддерживают перевод только на русский или английский."))
				return true
			}
		}
