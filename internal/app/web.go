...
func (w *WebAdmin) templateTagsJSON(rw http.ResponseWriter, r *http.Request) {
	...
	items := []item{
		...
		{Value: "{{ .gpt_4h_remaining }}", Label: "{{ .gpt_4h_remaining }} — остаток GPT-токенов автора в текущем 4-часовом окне"},
		{Value: "{{ .gpt_4h_limit }}", Label: "{{ .gpt_4h_limit }} — лимит GPT-токенов автора на 4 часа"},
		{Value: "{{ .gpt_4h_low_threshold }}", Label: "{{ .gpt_4h_low_threshold }} — порог усталости GPT-токенов автора"},
...
func settingsSchema() []settingField {
	return []settingField{
		...
		{Key: "ROLEPLAY_INLINE_ADULT_ENABLED", Label: "Roleplay inline: показывать adult-действия", Type: "bool", Description: "false"},
		...
		{Key: "GPT_TRANSLATE_MODEL", Label: "GPT translate: model", Type: "string", Description: "gpt-4.1"},
		{Key: "GPT_TRANSLATE_TRANSCRIBE_MODEL", Label: "GPT translate: transcription model", Type: "string", Description: "whisper-1", Options: []string{"whisper-1", "gpt-4o-mini-transcribe", "gpt-4o-transcribe"}},
		{Key: "GPT_TRANSLATE_TTS_MODEL", Label: "GPT translate: TTS model", Type: "string", Description: "gpt-4o-mini-tts"},
		{Key: "GPT_TRANSLATE_TTS_VOICE", Label: "GPT translate: TTS voice", Type: "string", Description: "marin"},
		{Key: "GPT_TRANSLATE_RESLANG", Label: "GPT translate: target language", Type: "string", Description: "ru"},
		{Key: "GPT_TRANSLATE_TIMEOUT_SEC", Label: "GPT translate: timeout (sec)", Type: "int", Description: "500"},
...
func iconForActionType(v model.ActionType) string {
	...
	case model.ActionTypeMediaTikTok:
		return "bi-tiktok"
	case model.ActionTypeMediaCoub:
